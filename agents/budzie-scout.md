...
2. **Refuse sensitive paths**: `.env*`, key/credential files, lockfiles, and
   ...
   not work around it.
3. **Reuse existing logic**: Run `node scripts/scout.mjs <scope> --json` to get
...
6. **Budget-metered**: Dispatch through `node scripts/agents.mjs dispatch --agent
...
2. **Audit**: For each scope, run `node scripts/scout.mjs <scope> --json`

...
- Runtime code is JavaScript annotated with JSDoc and a `// @ts-check` header.
  Type-checking runs as `tsc --noEmit` from dev-only dependencies. No
  install-time build step and no shipped TypeScript: hosts load the JS as-is.
  `dist/` may hold committed, hand-packaged distribution artifacts (e.g. a
  single-file skill bundle) generated by an explicit local script — never an
  install-time transpile.
- Engine code lives under `src/`. Host-facing surfaces a plugin loader discovers
  stay at the repo root: `commands/ skills/ agents/ hooks/ bin/`.
